...
class Mailbox(models.Model):
    ...
    users = models.ManyToManyField(
        ...
        blank=True,
        help_text=_(
            'The list of users allowed to search contents of this mailbox.'
        ...
        related_name="user_set_mb",
        related_query_name="user_mb"
    )
    ...
    def get_new_mail(self):
        ...
        mbdir = os.path.join(settings.BASE_DIR, 'apps', 'cores', 'incoming', 'django_mailbox', self.name)
        if not os.path.exists(mbdir):
            logger.info("Dir does not exists. Creating '%s'" % mbdir)
            os.makedirs(mbdir)
            logger.info("Created")
        
        mbdiratt = os.path.join(mbdir, 'attachments')
        if not os.path.exists(mbdiratt):
            logger.info("Dir does not exists. Creating '%s'" % mbdiratt)
            os.makedirs(mbdiratt)
            logger.info("Created")
        ...
        for message in connection.get_message():
            ...
            msg = self.process_incoming_message(message, mbdir, mid)
            new_mail.append(msg)
            mid = mid + 1   # update mid
        return new_mail
...
class MessageAttachment(models.Model):
    ...
    document = models.FileField(upload_to='django_mailbox/attachments/%Y/%m/%d/')
