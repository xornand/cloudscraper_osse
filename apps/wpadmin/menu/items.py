    ...
    def init_with_context(self, context):
        ...
        for model, perms in items:
            ...
            if app_label not in apps:
                ...
                # override app names 
                title = "app"
                if app_label.title() == "Django_Mailbox":
                    title = "Emails"
                elif app_label.title() == "Constance":
                    title = "System"
                else:
            ...
            # filter Message attachments and Messages models
            
            #print 'App label' + app_label
            #print 'Title' + unicode(capfirst(model._meta.verbose_name_plural))
            
            title = unicode(capfirst(model._meta.verbose_name_plural))
            
            if app_label == u'django_mailbox' and (title == u'Message attachments' or title == u'Messages'):
                pass
            else:
            ...
            if (app_label == 'cores' and not added):
                # manually add tasks item
                apps[app_label]['models'].append({
                    'title': capfirst('tasks'),
                    'url': '/adminpanel/tasks',
                    'add_url': '/adminpanel/tasks',
                    'description':
                    # Translators: This is already translated in Django
                    perms['change'] and _("Change") 
                    # Translators: This is already translated in Django
